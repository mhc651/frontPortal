...
<section class="login container--fluid" style="overflow: hidden">
  <div class="container ">
    ...
    <div class="login-main">
      <Tabs  :value="tagsname" style="height: 368px" v-model="tagsname" @on-click="getNowPane(tagsname)">
        <TabPane label="账号登录" name="name1">
            <Form ref="formInlines" :model="formInline" :rules="ruleInline" >
              <FormItem prop="user">
                <Input id="account" autofocus class="input" v-model="formInline.user" placeholder="账号"/>
              </FormItem>
            ...
            <!--</div>-->
              ...
              <Button :disabled='post' class="login_btn" type="primary" size="large" long @click="handleSubmit('formInlines')" v-on:keyup.enter="handleSubmit('formInlines')">
                立即登录
              </button>
              </FormItem>
        ...
        <TabPane label="CA登录" name="name2">
            ...
            <p class="login_ca_p">首次登录时，请下载并安装 “<a :href="caUrl" style="color: #1686EA">CA证书客户端</a>” 仅支持IE浏览器</p>
        </TabPane>
      </Tabs>
      <!-- ca end-->
  ...
  <div v-if="showMode">
    <ca-active :bdInfo='bandInfo' />
  </div>

  <Modal v-model="modalSonC" width="600" :closable="false" :mask-closable="false">
    <p>
    ...
    <div>
      <div  class="setsuctitle" style="line-height: 45px"><i style="float:left;font-size: 30px;color:#1ABC9C;margin-left: 120px;" class="icon hx hx-hx_zhengquexuanzhong"></i><span style="margin-left: -120px">已为您生成该机构子账号！</span></div>
      <div  class="setsuctip"><span>系统将在</span><span class="textred">{{fsecond}}</span>秒后自动关闭窗口</div>
    </div>
  </Modal>
...
import caActive from '../../components/CaActive'
...
export default {
  ...
  mounted() {
    this.caUrl = "http://bucket-aupfj.oss-cn-shanghai.aliyuncs.com/files/helpfile/SecSealCpt.zip"
    ...
    window.addEventListener(
      'keyup',
      e => {
        if (e.keyCode == 13 && this.tagsname=='name1') {
          this.handleSubmit('formInlines')
        }else if(e.keyCode == 13 && this.tagsname=='name2'){
          this.caLogin()
        }
      },
      false
  ...
  methods: {
    ...mapActions(['getqcode', 'login', 'verifPicCode', 'getRandom', 'caLogins', 'getOrgStatus', 'ysTlogin', 'pefMaLogin', 'validBindCa', 'loginOut']),
    //重新登陆
    relogin: function(){
      clearInterval(this.ftimer)
      this.modalSonC = false
      this.loginOut()
    ...
    IsValidBindCa() { /*未完善资料判断是否绑定CA*/
      this.validBindCa().then(
        res => {
          ...
          if(res){
            switch (res.content.status) {
              case 'ORG_AUDIT_PASS':/*机构已审核通过*/
                if(res.content.isEnterprise == 1){//1:企业用户,0:非企业
                  window.location.href = '/dws/uas/portal/jump/hcsb2'
                }else{
                  this.$router.push('/app')
                }
              ...
              case 'ORG_WAIT_AUDIT':/*处于待审核，请等待*/
                ...
                break
              case 'ORG_NOT_AUDIT_PASS':/*审核不通过，请重新提交*/
                if(res.content.isEnterprise == 1){//1:企业用户,0:非企业
                  window.location.href = '/dws/uas/portal/jump/hcsb2'
                }else{
                  window.location.href = '/dws/ocs/main/menu'
                ...
                break
              case 'ORG_NOT_COMMIT_APPLY':/*未提交资料，请完善资料*/
                if(res.content.isEnterprise == 1){//1:企业用户,0:非企业
                  window.location.href = '/dws/uas/portal/jump/hcsb2'
                }else{
                  window.location.href = '/dws/ocs/main/menu'
        ...
        err => {
          if(err.errmsg=='UNBOUND_CA'){/*未绑定CA认证*/
            this.$router.push('/compBindCa')
            this.getOrgStatus().then(res => {
            })
          }else if(err.errmsg == 'ONLYACCOUNT'){/*只允许账号登陆*/
            this.ca_error = err.errcode
          }else {/*已绑定CA认证，只允许CA登陆*/
            this.tagsname = 'name2'
            this.ca_error = "已绑定CA认证，只允许CA登陆"
          }
        }
      )
    },
    ...
    loginInfo() {
      ...
      this.login({
        ...
        res => {
          if (res) {
            switch (res.status) {
              case 'ONLY_BINDING_CA':/*只能CA登录*/
                this.IsValidBindCa()
                //this.$router.push('/compBindCa')
                break
              case 'ONLY_LOGIN_CA':/*该账号只支持CA登录*/
                this.tagsname = 'name2'
                this.ca_error = "已绑定CA认证，只允许CA登陆"
                break
              case 'ORG_WAIT_AUDIT':/*处于待审核，请等待*/
                if(res.isEnterprise == 1){//1:企业用户,0:非企业
                  window.location.href = '/dws/uas/portal/jump/hcsb2'
                }else{
                  window.location.href = '/dws/ocs/main/menu'
                }
              ...
              case 'ORG_AUDIT_PASS':/*机构已审核通过*/
                ...
                break
              case 'NOT_BINDING_CA':/*企业或管理单位的主账号必须绑定CA*/
                this.$router.push('/compBindCa')
                break
              default:
                //this.$router.push('/review')
    ...
    conCaLoginUrl(updata) {
      if(updata){
        this.showMode = false
        this.caLogins(updata).then(
          res => {
            //console.log("ca login info: "+JSON.stringify(res))
            if(res.content.status!="CA_CERT_UNBINDING" && res.content.status!="PRODUCTCHILDACCOUNT"){
              this.IsValidBindCa()
            }
            if(res.content.status == "CA_CERT_UNBINDING"){//1:“UKey”未在系统注册！
              console.log("登录返回信息:"+JSON.stringify(res.content))
              this.showMode = true
              this.bandInfo = res.content
              this.ca_error = "“UKey”未在系统注册！";
            }
            if(res.content.status == "PRODUCTCHILDACCOUNT"){ //2:该KEY没有绑定账号，已重新生成子账号
              console.log("PRODUCTCHILDACCOUNT登录返回信息:"+JSON.stringify(res.content))
              this.modalSonC = true
              this.fiveCountd()
            }
          },
          err => {
            console.log(err)
            if(err.errmsg=='ORGNUMEXIET'){
              this.ca_error = "组织机构代码证已经存在";
            }else if(err.errmsg=='CA_CERT_EXPIRED'){
              this.ca_error = "UKey证书已过期";
            }else if(err.errmsg=='CA_ERROR_LOGIN'){
              this.ca_error = "登录KEY出错！";
            }else if(err.errmsg=='CA_CERT_ERROR'){
              this.ca_error = "证书错误！";
            }else if(err.errmsg=='USER_DISABLED'){
              this.ca_error = "用户被禁用";
            }else if(err.errmsg=='ORG_NOT_FOUND'){
              this.ca_error = "机构不存在";
            }else if(err.errmsg=='ORG_NOT_VALID'){
              this.ca_error = "机构已过期";
            }else if(err.errmsg=='ORG_FORBIDED'){
              this.ca_error = "机构被禁用";
            }else if(err.errmsg=='KEYISSECOND'){
              this.ca_error = "机构主账号未激活";
            }else if(err.errmsg=='UNKOWN_CODE'){
              this.ca_error = "获取证书机构名称失败";
            }else{
              this.ca_error = err.errcode
            }
        ...
        if(browVs == 'edge' || browVs == -1 ){
          ...
          socket.onmessage = function (msg)
          {
            if (typeof msg.data == "string")
            {
              ...
              if(updata == 1){
                me.ca_error = "未检测到UKey或Ukey未登陆"
                return
              }
              console.log("上传的参数:"+JSON.stringify(updata))
              me.conCaLoginUrl(updata)
            }
          }
          ...
          {
            restatus = 1
            me.ca_error = "您未使用新版证书或未正确安装证书驱动";
          }
        }else{/*IE浏览器*/
