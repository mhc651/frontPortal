...
const actions = {
  ...
  // 完善资料登陆
  pefMaLogin({ commit }, payload) {
    return callApi('/dws/uas/login/judgement', 'POST', payload).then(res => {
      if (res.success) {
        commit({
          type: types.LOGIN,
          content: res.content
  ...
  login({ commit }, payload) {
    return callApi('/dws/uas/login/login', 'POST', payload).then(res => {
      if (res.success) {
        if (res.content.status !='ONLY_BINDING_CA' && res.content.status !='ONLY_LOGIN_CA') {
          commit({
            type: types.LOGIN,
            content: res.content
          })
  ...
  getOrgStatus({ commit }) {
    return callApi('/dws/uas/register/audit', 'POST', {}, true).then(res => {
      //console.log("用户机构信息1: "+JSON.stringify(res))
      if (res.success) {
        commit({
  ...
  bindCa: function({ commit }, payload) {
    console.log("绑定上传参数: "+ JSON.stringify(payload))
    let url = ''
    ...
    return callApi(url, 'POST', payload).then(res => {
      if (res.success) {
        console.log("bindca success info is: "+res)
        return Promise.resolve(res)
      } else {
        console.log("bindca fail info is: "+res)
        return Promise.reject(res)
      }
    })
  },
  ...
  //激活CA
  activeCa: function({ commit }, payload) {
    console.log("ca激活上传参数: "+ JSON.stringify(payload))
    let url = '/dws/uas/register/carigister'
    return callApi(url, 'POST', payload).then(res => {
      if (res.success) {
        console.log("activeCa success info is: "+res)
        return Promise.resolve(res)
      } else {
        console.log("activeCa fail info is: "+res)
        return Promise.reject(res)
      }
    })
  },
